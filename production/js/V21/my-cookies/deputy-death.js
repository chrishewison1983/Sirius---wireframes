...
$("#confirm-deputy-death").on("click", function (e) {
     ...
     $.cookie("deputy-death-how-notified", $("#deputy-death-how-notified").val(), {path:'/'});
     $.cookie("deputy-death-proof", $("input[name=deputy-death-proof]:checked").val(), {path:'/'});
     $.cookie("deputy-death-letter-sent", $("#datepicker").val(), {path:'/'});
...
$('#deputy-has-died').hide();

$('.deputy-status-title').css('display', 'inline-block');
...
if ($.cookie('deputy-death') == 'true' && $.cookie('deputy-death-proof') == 'No') {
     ...
     $('#section-header.deputy-page').addClass('deceased-message');
     $('#section-header.deputy-page .type').hide();
     $('#section-header.deputy-page h1 .normal-status').hide();
     $('#section-header.deputy-page h1 .deceased').show();
     $('#section-header.deputy-page h1 .deceased').text('Deputies death notified');
     $('.deputy-status-title').hide();

     $('.deputy-death-button .deputy-death').html('<span></span>Confirm death');

     $('.death-text').text('Record proof of death notification');

     $('#confirm-deputy-death').addClass('de-activate');

     // Changes table items
     $('#deputies-table tr[data-value="1"]').addClass('error-row');
     $('#deputies-table tr[data-value="1"] .deputy-status-death').text('Deceased');
     ...
     $('.deputy-status-title').hide();
     $('.deputy-death-button .deputy-death').hide();
     ...
     // $('.deputy-death-button .deputy-death').html('<span></span>View death details');
     // $('.info-cant-change').addClass('read-only');
     // $('.info-cant-change input').attr('disabled', 'disabled');
...
//      $('.deputy-death-button .deputy-death').html('<span></span>Record death');
//
//      // Changes the confirmation text
...
if ($.cookie("deputy-death-how-notified")) { $("#deputy-death-how-notified").val($.cookie("deputy-death-how-notified")); }
