...
(function (factory) {
    ...
    Autocomplete.formatResult = function (suggestion, currentValue) {
        var reEscape = new RegExp('(\\' + ['/', '.', '*', '+', '?', '|', '(', ')', '[', ']', '{', '}', '\\'].join('|\\') + ')', 'g'),
            pattern = '(' + currentValue.replace(reEscape, '\\$j1') + ')';

        return suggestion.value.replace(new RegExp(pattern, 'gi'), '<strong>$j1<\/strong>');
    };
    ...
    Autocomplete.prototype = {
        ...
        initialize: function () {
            ...
            container.on('click.autocomplete', suggestionSelector, function () {
                that.select($j(this).data('index'), false);
            });
